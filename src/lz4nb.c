...
* @param bWait File I/O 작업 시, 대기 여부 (동기: TRUE, 비동기: FALSE)
...
BOOL LZ4F_NB_Process(LZ4_NB_Context_t* lz4nbCtx) {
    ...
    for (DWORD chunk = 0; chunk < lz4NB->dwTotalChunks; chunk++) {
        bResult = async_read(
            ...
            &dwBytesRead, &(lz4nbCtx->readOverlap), lz4NB->bWait
...
* @return 압축 성공 여부
*/
BOOL compress_lz4(const char* inputFilePath, const char* outputFilePath) {
    BOOL bResult = FALSE;
    ...
    LZ4_NB_Core_t* lz4NB;
    if (LZ4F_createNB(&lz4NB, hInput, hOutput, CHUNK_SIZE, dwTotalChunks, TRUE)) {
        bResult = LZ4F_NB_Compress(lz4NB);
    } else {
        log_message("error : resource allocation failed.");
    }
