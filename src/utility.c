 ...
 * @return const char* 압축 알고리듬에 해당하는 확장자
 */
const char* get_extension(CompressionAlgorithm algorithm) {
    switch (algorithm) {
    case LZ4:
        return ".lz4";
    case ZSTD:
 ...
 * @param algorithm 압축 알고리듬
 * @return char* 생성된 새로운 파일 이름
 */
char* get_output_file_name(const char* filename, CompressionAlgorithm algorithm) {
