...
## 개요
이 프로젝트는 Embedded 환경에서 안정적인 압축을 수행하기 위한 예제 코드입니다.  
- **Non-Blocking 방식**을 사용하여 주요 프로세스의 지연을 최소화  
...
- [Compression for Embedded Systems](#compression-for-embedded-systems)
  ...
  - [왜 Non-Blocking, Low Resource, High-Speed 압축이 필요한가?](#왜-non-blocking-low-resource-high-speed-압축이-필요한가)
    - [Embedded 환경에서 압축이 필요한 이유](#embedded-환경에서-압축이-필요한-이유)
    - [Non-Blocking이 필요한 이유](#non-blocking이-필요한-이유)
      - [왜 Non-Blocking이 중요한가?](#왜-non-blocking이-중요한가)
      ...
      - [동기(Synchronous) vs 비동기(Asynchronous), Blocking vs Non-Blocking](#동기synchronous-vs-비동기asynchronous-blocking-vs-non-blocking)
        - [개념 설명](#개념-설명)
        - [UML 예시](#uml-예시)
      - [비동기, Non-Blocking 구현하는 방법 (설계 패턴, Win API 등 활용)](#비동기-non-blocking-구현하는-방법-설계-패턴-win-api-등-활용)
...
### Non-Blocking이 필요한 이유  
**Embedded System은 Resource가 제한된 환경에서 실시간으로 동작**해야 하므로, 중요도가 낮은 작업이 높은 작업을 배제하며 System을 점유하지 않도록 **Non-Blocking 방식**이 필수적입니다.

#### 왜 Non-Blocking이 중요한가?  
- **Kernal Mode는 한정적**이며, 사용 가능한 Resource가 모두 소진될 경우 프로세스가 <strong>대기 상태(Blocking)</strong>가 됨.  
- 만약 **중요한 프로세스**가 대기 상태로 들어가면, System 전체가 지연되거나 심각한 장애가 발생할 수 있음.  
- 따라서, 압축과 같이 상대적으로 **우선순위가 낮은 작업**은 **Non-Blocking 방식**으로 처리하여 주요 프로세스의 실행을 방해하지 않아야 함.  
...
#### 비동기, Non-Blocking 구현하는 방법 (설계 패턴, Win API 등 활용)
...
## 결론  
Embedded 환경에서 **Non-Blocking 방식의 압축 기법**을 사용하면, 아래와 같은 이점을 얻을 수 있습니다.
- **주요 프로세스의 안정성 유지**  
- **System 지연 최소화**  
- **제한된 리소스를 효율적으로 활용**  
